...
for i = 1:k
    ...
    idx = sort(randperm(length(ECG), N));    % índices aleatórios exclusivos 
    ECG = ECG(idx);                          % seleção das amostras
...
M_erro = normalize(mean(erro,2)); D_erro = normalize(std(erro,0,2));
...
subplot(2,1,2)
plot(erro,'k')           %Erro de reconstrução
set(gca,'xlim',[0 1024]);
xlabel('Time (s)');
ylabel('Error (V)');
